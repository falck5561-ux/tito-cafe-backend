...
  try {
    const query = `SELECT p.id, p.fecha, p.total, p.estado, p.tipo_orden, p.direccion_entrega, p.latitude, p.longitude, p.referencia, p.telefono, u.nombre AS nombre_cliente, (SELECT json_agg(json_build_object('nombre', pr.nombre, 'cantidad', dp.cantidad, 'precio', dp.precio_unidad,'opciones', CASE WHEN dp.opciones IS NOT NULL THEN dp.opciones::json ELSE '[]'::json END)) FROM detalles_pedido dp JOIN productos pr ON dp.id_producto = pr.id WHERE dp.id_pedido = p.id) AS productos FROM pedidos p LEFT JOIN usuarios u ON p.id_cliente = u.id WHERE p.tienda_id = $1 ORDER BY p.fecha DESC;`;
...
  try {
    const query = `SELECT p.id, p.fecha, p.total, p.estado, p.tipo_orden, p.telefono, (SELECT json_agg(json_build_object('nombre', pr.nombre, 'cantidad', dp.cantidad, 'precio', dp.precio_unidad,'opciones', CASE WHEN dp.opciones IS NOT NULL THEN dp.opciones::json ELSE '[]'::json END)) FROM detalles_pedido dp JOIN productos pr ON dp.id_producto = pr.id WHERE dp.id_pedido = p.id) AS productos FROM pedidos p WHERE p.id_cliente = $1 AND p.tienda_id = $2 ORDER BY p.fecha DESC;`;
